...
- Generation numbers, in two places: `useEngine` (engines.js) gives every engine request a number; a build that
  finishes after a newer request closes itself and its failure moves nothing. `startCamera`/`stopCamera`
  (camera.js) do the same for the camera, so two starts in flight never leave a stream open. After a camera
  start or a picture-size change `backend.reset()` (threads engine) drops results still in flight from the
  old stream.
- Engine-error cards carry `dataset.kind = 'card'`, camera cards `'camera'`; an engine load never replaces a
  camera card (it holds the way out).
- The 3D view is drawn on demand: `requestRender()` (skeleton.js) is called by `present`, `resize`, `frontView`,
  the display switch, the character and the phone model arriving; the loop draws when it was requested, when the
  orbit controls moved, or once a second as a safety net. Anything new that changes the scene outside `present`
  must call `requestRender()`, or it shows up to a second late.
<!-- hand-written: end -->
...
| `js/engines.js` | 211 | recognition: the local store for the big files (IndexedDB), the MediaPipe engines (three separate models or the combined one), hands tied to the body's wrists, and the engine management (loading card, the automatic chain, error cards). |
| `js/threads.js` | 168 | threads: recognition in worker threads. With enough cores the three models (face, hands, body) each run in their own thread; with two or three cores one thread runs all three in turn; without workers the main thread does it as before. The main thread hands every idle thread a copy of the current frame and draws with the newest results it has, so the picture never waits for recognition. Each thread reports its work to the Performance panel. |
| `js/skeleton.js` | 217 | skeleton: the 3D scene (renderer, cameras, lights, grid), the rigs of joints and bones for the body, the hands and the face, image/world → scene coordinates, smoothing, the puppet update per frame, and the sizing of the 3D canvas to its box. |
...
| `js/character.js` | 372 | character: his textured AccuRIG model, loaded on demand; directional retargeting of the smoothed landmarks onto its bones (Character 3D); the On-camera fit in image space with unrecognised regions hidden by a per-vertex mask; the display switch skeleton / character / on camera; the rest-pose button. |
| `js/phone.js` | 186 | phone: the cell-phone detector in its own worker (fed with bytes from the local store), which hand holds, and the phone (his iPhone model, a slab until it loads) placed from the hand points in every view. |
...
- L127 `fn` `assignHands(hands, pose)` — With three separate models the hands are tied to the nearest wrist of the body (in image space); a hand that has no wrist nearby keeps its o

**engine management**

- L151 `const` `CHAIN` — what "automatic" tries, in order
- L152 `const` `engineChain` — no threads on this device: skip them
- L153 `let` `backend, chainPos, wanted, gen` — gen: the newest engine request wins, older builds close themselves
- L154 `fn` `showLoading(html, kind)`
- L160 `fn` `showCard(title, reason, buttons, kind)` — A card keeps the menu reachable and always offers a way out; long traces stay in the console.
- L165 `fn` `showError(title, e)`
- L169 `async fn` `useEngine(sel, rebuild)` — rebuild: the same key again (a new thread plan)
- L199 `fn` `onDetectError(e)`
- L205 `on` `ui.engine.onchange`
- L206 `on` `$('optThreads').onchange`
...
- imports from: `@mediapipe/tasks-vision` (1: FilesetResolver); `core` (5: MPV, MODEL, IS_MOBILE, $, ui); `engines` (2: bigFile, assignHands); `monitor` (1: mon)
- exports: `threadPlan`, `createThreadedBackend`, `grabFrame`

- L11 `const` `THREAD_NAME`
- L18 `fn` `threadPlan()` — How many recognition threads this machine should run: the Threads menu, else by the core count. Measured 2026-09-12 (laptop iGPU): three GPU
- L29 `const` `landmarkerWorkerSrc` — The thread's own code (a classic worker: the library needs importScripts). It receives the library, the wasm pair and its models as bytes, b
- L30 `run` `"const tasks = {}; let used = '';\n" +`
- L31 `run` `"{ let gl = false;   // answered at once, before any bytes are sent: a browser without Web`
- L32 `run` `"  try { const c = typeof OffscreenCanvas !== 'undefined' && new OffscreenCanvas(1, 1), x `
- L33 `run` `"  self.postMessage({ type: 'hello', gl }); }\n" +`
- L34 `run` `"const make = async (V, fs, kind, buf, delegate) => {\n" +`
- L35 `run` `"  const base = { baseOptions: { modelAssetBuffer: new Uint8Array(buf), delegate }, runnin`
- L36 `run` `"  if (typeof OffscreenCanvas !== 'undefined') base.canvas = new OffscreenCanvas(1, 1);   `
- L37 `run` `"  if (kind === 'face') return V.FaceLandmarker.createFromOptions(fs, Object.assign(base, `
- L38 `run` `"  if (kind === 'hand') return V.HandLandmarker.createFromOptions(fs, Object.assign(base, `
- L39 `run` `"  return V.PoseLandmarker.createFromOptions(fs, Object.assign(base, { numPoses: 1 }));\n"`
- L40 `run` `"};\n" +`
- L41 `run` `"const warmUp = t => { t.detectForVideo(new ImageData(64, 64), 1); };   // the first detec`
- L42 `run` `"self.onmessage = async e => {\n" +`
- L43 `run` `"  const m = e.data;\n" +`
- L44 `run` `"  if (m.type === 'init') {\n" +`
- L45 `run` `"    try {\n" +`
- L46 `run` `"      importScripts(URL.createObjectURL(new Blob([m.lib], { type: 'text/javascript' })));`
- L47 `run` `"      const V = self.exports;\n" +`
- L48 `run` `"      const fs = { wasmLoaderPath: URL.createObjectURL(new Blob([m.js], { type: 'text/jav`
- L49 `run` `"      for (const kind of m.kinds) { tasks[kind] = await make(V, fs, kind, m.models[kind],`
- L50 `run` `"      self.postMessage({ type: 'ready', delegate: used });\n" +`
- L51 `run` `"    } catch (err) { self.postMessage({ type: 'error', message: String((err && err.message`
- L52 `run` `"    return;\n" +`
- L53 `run` `"  }\n" +`
- L54 `run` `"  if (m.type === 'frame') {\n" +`
- L55 `run` `"    const out = { type: 'result', ts: m.ts, ms: {} };\n" +`
- L56 `run` `"    try {\n" +`
- L57 `run` `"      for (const kind of m.kinds) {\n" +`
- L58 `run` `"        const t = performance.now(), r = tasks[kind].detectForVideo(m.frame, m.ts);\n" +`
- L59 `run` `"        out.ms[kind] = performance.now() - t;\n" +`
- L60 `run` `"        if (kind === 'face') { out.face = r.faceLandmarks[0] || null; out.blend = r.faceB`
- L61 `run` `"        else if (kind === 'hand') out.hands = r.landmarks.map((img, i) => ({ side: null, `
- L62 `run` `"        else { out.pose = r.landmarks[0] || null; out.poseWorld = r.worldLandmarks[0] || `
- L63 `run` `"      }\n" +`
- L64 `run` `"    } catch (err) { out.error = String((err && err.message) || err); }\n" +`
- L65 `run` `"    m.frame.close(); self.postMessage(out);\n" +`
- L66 `run` `"  }\n" +`
- L67 `run` `"};\n" +`
- L68 `run` `"})();\n";`
- L71 `fn` `grabFrame(v)` — The frame handed to a thread: a copy of the camera picture that can be moved to another thread.
- L73 `async fn` `startWorker(kinds, delegate, loadKit)` — loadKit(): the bytes, read only once the thread has said hello
- L97 `async fn` `createThreadedBackend(delegate)` — The engine: same shape as the others ({ key, label, detect(v, ts), close() }), but detect() never waits.
...
- exports: `avatar`, `view`, `renderer`, `scene`, `camera`, `overlayCamera`, `isOverlay`, `renderCamera`, `controls`, `frontView`, `grid`, `body`, `handRig`, `neck`, `clavL`, `clavR`, `spine`, `rootJoint`, `chinJoint`, `pelvisJoint`, `spineJoints`, `head`, `faceGroup`, `facePos`, `sm`, `resetSmoothing`, `updateBody`, `updateHand`, `updateFace`, `updateTorso`, `resize`, `requestRender`, `takeRender`
...
- L29 `let` `renderWanted` — The 3D view is drawn only when something changed: a new pose (present), a turn of the view, a new size, a new display, the character or the 
- L30 `const` `requestRender`
- L31 `const` `takeRender`
- L32 `fn` `frontView()`
- L42 `run` `frontView();`
- L43 `run` `scene.add(new THREE.HemisphereLight(0xdfe9f3, 0x1a2027, 1.6));`
- L44 `const` `sun`
- L45 `const` `grid`
- L47 `const` `sphereGeo`
- L48 `const` `cylGeo`
...
- L50 `const` `matJoint`
- L51 `const` `_up, _dir`
- L52 `fn` `setBone(mesh, a, b, r)`
- L62 `fn` `makeRig(jointIds, bones, nPts, color, rJoint, rBone)` — A rig: joints (spheres) + bones (cylinders) over a point table `pts`
- L76 `const` `body`
- L77 `const` `handRig`
- L79 `const` `torsoMat, torsoJointMat`
- L80 `const` `mkBone`
- L81 `const` `mkJoint`
- L82 `const` `neck, clavL, clavR, spine`
- L83 `const` `rootJoint, chinJoint, pelvisJoint`
- L84 `const` `spineJoints`
- L85 `const` `head`
- L86 `let` `lastPW` — the last pose (world) result, for the visibility of its points
- L89 `const` `facePos` — The face: 478 shared points, drawn as the mesh (thin), the contours (bright) and dots
- L90 `const` `faceAttr`
- L91 `const` `mkGeo`
- L92 `const` `faceGroup`
- L93 `const` `tessLines`
- L94 `const` `contLines`
- L95 `const` `facePoints`
- L96 `run` `for (const o of [tessLines, contLines, facePoints]) { o.frustumCulled = false; faceGroup.a`

**coordinates**

- L102 `const` `_a, _b, _t` — Image landmarks: x,y in 0..1 of the frame, z in the same units as x, smaller = closer to the camera. World landmarks (body, hands): metres, 
- L103 `fn` `imgToScene(p, k, out)` — point in the frame -> a plane facing the viewer, k = metres per frame width
- L107 `fn` `deltaToScene(dx, dy, dz, out)` — a metric offset in world axes -> scene axes
- L113 `const` `sm` — Smoothing: an exponential average per point; a part that reappears starts fresh.
- L115 `fn` `smoothInto(store, i, v)`
- L120 `fn` `resetSmoothing()`

**puppet update**

- L123 `fn` `updateBody(res)`
- L142 `fn` `updateHand(h)`
- L157 `fn` `updateFace(res)`
- L175 `fn` `updateTorso(hasBody, hasFace)`
- L199 `fn` `resize()`
- L212 `on` `ResizeObserver`
- L213 `on` `ResizeObserver`
...
- imports from: `three` (1: THREE); `core` (9 names); `engines` (1: bigFile); `skeleton` (23 names); `eyes` (2: eyes, updateEyes); `camera` (1: stopCamera); `main` (1: present)
...
- L365 `on` `$('optGuides').onchange`
- L366 `on` `$('avatarRest').onclick`
...
- imports from: `three` (1: THREE); `@mediapipe/tasks-vision` (2: FilesetResolver, ObjectDetector); `core` (8 names); `engines` (2: bigFile, getFileset); `skeleton` (5: avatar, scene, isOverlay, handRig, requestRender); `monitor` (1: mon)
...
- L134 `fn` `dressPhone(g)`
- L138 `fn` `phoneMesh(side)` — sized in units of the hand length: 1.47 x 0.72 x 0.08 (14.7 x 7.2 x 0.8 cm for a 10 cm hand)
- L148 `const` `_pc, _pl, _ps, _pn, _pf, _pm`
- L149 `fn` `phoneFrame(pts)` — pts: 21 Vector3-like points -> { c, long, short, normal, L }
- L158 `fn` `holding(side)`
- L161 `fn` `updatePhones(res)` — The phone is a fixed size (1.47 hand lengths long) and always in the hand: on the palm side, tilted with the palm, its long edge along which
- L184 `on` `$('optPhone').onchange`
...
- imports from: `core` (7 names); `engines` (2: showCard, backend); `skeleton` (8 names); `character` (1: avatarUI); `main` (1: setStatus)
- exports: `startCamera`, `stopCamera`, `listCameras`, `syncSize`, `setMirror`

- L10 `let` `stream, currentDeviceId, currentFacing, restarts, camGen` — restarts: when the browser ended a track and we started again; camGen: the latest start or stop wins
- L11 `on` `window 'orientationchange'`
- L12 `async fn` `startCamera(deviceId, facing)`
- L61 `fn` `stopCamera()`
- L66 `async fn` `listCameras()`
- L78 `fn` `syncSize()` — The picture size is read whenever it changes (a phone turned, a camera that starts with a tiny first frame).
- L85 `on` `video 'loadedmetadata'`
- L86 `on` `video 'resize'`
- L87 `on` `ui.camSel.onchange`
- L88 `on` `ui.btnCam.onclick`

**controls**

- L91 `fn` `setMirror(on)`
- L92 `on` `ui.mirror.onchange`
- L93 `on` `ui.btnFlip.onclick`
- L94 `on` `ui.video.onchange`
- L95 `on` `$('front').onclick`
- L96 `on` `$('btnTools').onclick`
- L97 `on` `$('exprTitle').onclick`
- L99 `on` `ui.btnView.onclick` — On a phone there is one view at a time: the camera picture or the puppet
- L107 `on` `ui.btnSnap.onclick`
...
- L11 `const` `mon`
- L99 `const` `resetGpu` — a lost context takes its queries and extensions with it
- L100 `on` `renderer.domElement 'webglcontextlost'`
- L101 `on` `renderer.domElement 'webglcontextrestored'`
- L102 `fn` `initGpu()`
- L114 `on` `$('perfTitle').onclick`

## `js/main.js`

the loop: one frame (recognition → 2D drawing → puppet → character → phone), the status line, the console handle for the tests, and the start-up. The modules import each other in circles (a function defined in a later file is called from an earlier one). That is safe because no module touches another module's variables while the files are still loading: at load time each file only builds its own objects and points event handlers at functions; the calls come later.

- imports from: `three` (1: THREE); `core` (8 names); `engines` (4: backend, wanted, useEngine, onDetectError); `skeleton` (18 names); `eyes` (1: eyes); `character` (3: loadAvatar, updateAvatar, configureDisplay); `phone` (4: phone, acceptBoxes, detectPhones, updatePhones); `overlay` (2: draw2D, updateBlend); `camera` (3: startCamera, stopCamera, syncSize); `monitor` (1: mon); `threads` (1: threadPlan)
...
- L40 `let` `fps, frames, fpsAt, lastVT, lastTs, renderedAt`
- L41 `const` `perf` — ms per frame: recognition, then drawing (smoothed)
- L42 `fn` `setStatus(msg)`
- L48 `fn` `loop()`
- L77 `run` `window.mirrorPuppet = { THREE, state, phone, eyes, mon, threadPlan, acceptBoxes, present, ` — a small handle for testing from the console
- L79 `run` `configureDisplay(); loadAvatar().catch(() => {});`
- L80 `run` `loop();`
- L81 `run` `useEngine(ui.engine.value);`
- L82 `run` `startCamera();`
- L85 `const` `BUILD` — the build stamp (the ?v= of this file, see tools/stamp.py) shown in Settings, so a phone screenshot tells which build it runs
- L86 `run` `$('build').textContent = 'build ' + BUILD; window.mirrorPuppet.build = BUILD;`
