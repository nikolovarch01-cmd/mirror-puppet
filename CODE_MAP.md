...
- L39 `let` `fps, frames, fpsAt, lastVT, lastTs`
- L40 `const` `perf` — ms per frame: recognition, then drawing (smoothed)
- L41 `fn` `setStatus(msg)`
- L47 `fn` `loop()`
- L74 `run` `window.mirrorPuppet = { THREE, state, phone, eyes, mon, threadPlan, acceptBoxes, present, ` — a small handle for testing from the console
- L76 `run` `configureDisplay(); loadAvatar().catch(() => {});`
- L77 `run` `loop();`
- L78 `run` `useEngine(ui.engine.value);`
- L79 `run` `startCamera();`
- L82 `const` `BUILD` — the build stamp (the ?v= of this file, see tools/stamp.py) shown in Settings, so a phone screenshot tells which build it runs
- L83 `run` `$('build').textContent = 'build ' + BUILD; window.mirrorPuppet.build = BUILD;`
